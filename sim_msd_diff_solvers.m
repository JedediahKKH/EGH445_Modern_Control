...
[o1,eigA,o3]=eig(A);
idxA=find(eigA);
min_real_value_A=min(abs(real(eigA(idxA))));
simulated_settling_time=5;
stoptime =(1/min_real_value_A)*simulated_settling_time;
...
b=100;
B=[0,1;(-1/(c*m)),(-b/m)];
[output1,eigB,output3]=eig(B);
%% Fixed time steps and stop time
idxB=find(eigB);
min_real_value_B=min(abs(real(eigB(idxB))));
max_real_value_B=max(abs(real(eigB(idxB))));
simulated_settling_time=5;
simulated_step_size_gain=10;
h5=(1/max_real_value_B)/simulated_step_size_gain;
stoptime =(1/min_real_value_B)*simulated_settling_time;
